// STRUCT-ENUM KODLARINA DEVAM EDİLİYOR
// ---------------------------------
...
contract StructEnum { //sipariş alma süreci oluşturalım
    ...
    function createOrder(string memory _zipCode, uint256[] memory _products) checkProducts(_) external returns(uint256) { //returns(uint256)=sipariş ID'si dönsün 
        require(_products.length > 0, "Urun yok."); //parametrelerin boş olmamasının kontrolü
    ...
    function updateZip(uint256 _orderId, string memory _zip) checkOrderId(_orderId) external { //varış noktasını değiştirme
        //require(_orderId < orders.length, "Order Id gecersiz.");
...
//--------------------------------------------------------------------------------------
    modifier checkProducts(uint256 _products){
        require(_products.length>0, "Urun yok");//yukarılarde yazılmış kontrolü tekrar buraya yapıştırdık 
        _; //MODIFIER'DE BOYLE KAPATILMASI GEREKİYOR!!!!!
        //Daha sonra Modifier'in parametresini de yukarı çıkıp "createOrder" fonksiyonunun "checkProducts" parametresine (_) şeklinde yerleştirdik. 
    }
     modifier checkOrderId(uint256 _orderId) {
        require(_orderId < orders.length, "Gecersiz order ID.");
        _;
    }
    ...
    modifier onlyOwner {
        require(owner == msg.sender, "Gecersiz kimlik.");
        _;
    }

    modifier onlyCustomer(uint256 _orderId) {
        require(orders[_orderId].customer == msg.sender, "Siparisin sahibi degilsiniz.");
        _;
    }
}
